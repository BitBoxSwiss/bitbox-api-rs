            ...
            <form className="verticalForm" onSubmit={submitForm}>
                <label>
                    ...
                    <select value={network}
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => setNetwork(e.target.value as bitbox.CardanoNetwork)}>
                        {networkOptions.map(option => <option key={option} value={option}>{option}</option>)}
                    </select>
                </label>
                <label>
                    Account
                    <input type='number' min='0' value={account}
                           onChange={(e: ChangeEvent<HTMLInputElement>) => setAccount(Number(e.target.value))}/>
                </label>
                <label>
                    Change
                    <input type='checkbox' checked={isChange}
                           onChange={(e: ChangeEvent<HTMLInputElement>) => setIsChange(e.target.checked)}/>
                </label>
                <label>
                    Address Index
                    <input type='number' min='0' value={addressIndex}
                           onChange={(e: ChangeEvent<HTMLInputElement>) => setAddressIndex(Number(e.target.value))}/>
                </label>
                <label>
                    Display
                    <input type='checkbox' checked={display}
                           onChange={(e: ChangeEvent<HTMLInputElement>) => setDisplay(e.target.checked)}/>
                </label>
    ...
    return (
        <div>
            ...
            <form className="verticalForm" onSubmit={submitForm}>
                <label>
                    <select value={txType}
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => setTxType(e.target.value as TxType)}>
